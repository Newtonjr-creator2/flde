...
## 12. Phase 2C — Android-hosted runtime verification

**What existed before this phase:** `NativeRuntimeEnvironment` already ran
on the real Android device (it's not a CI-only abstraction) — the CI
workflow's `flutter test` just happens to execute the same *code* on
Ubuntu, which is a different question from "does it work on the phone."
The terminal, however, was calling `runtime.start(executable, args, ...)`
with the bare first token of whatever the user typed — never actually
invoking a shell. That mismatched the intended
`Terminal -> RuntimeEnvironment -> Shell -> ProcessExecutor` architecture
and meant shell builtins (`pwd` is one on Android's toybox — not
guaranteed to exist as a standalone `/system/bin/pwd` binary on every
device) and any quoting were unreliable or simply broken.

**What was missing:**
1. Nothing routed terminal commands through a real shell.
2. No dedicated, on-device, human-checkable verification path distinct
   from the automated CI tests — CI green does not mean "works on your
   phone," and section 12's very existence is because that distinction
   matters.
3. No Android-specific (`integration_test`) test scaffolding, only
   Ubuntu-runner unit/integration tests.

**What was implemented:**
- `TerminalSession.execute` now runs `<resolved shell> -c "<command>"`
  for everything except `cd` (kept as Dart-side state since each shell
  invocation is a fresh process). This is a real behavior change, not
  cosmetic — it fixes `pwd` and quoting, and gives real shell exit codes
  (e.g. 127) for unknown commands instead of a Dart-side guess.
- `RuntimeVerificationService` — runs exactly `uname -m`, `echo hello`,
  `pwd`, and a deliberately nonexistent command, via the real resolved
  shell, and returns the raw, unmodified `ExecResult` for each.
- A dedicated **Runtime Verification** screen (▶ icon on the home
  screen) that runs the above live and displays real stdout/stderr/exit
  code per command — this is the practical, adb-free verification path
  for a Termux-only, no-laptop workflow.
- `integration_test/runtime_verification_test.dart` — a genuine on-device
  test (compiles into the real APK, runs via `adb`), explicitly **not**
  wired into the GitHub Actions workflow (no Android emulator step exists
  there — adding one is a separate, heavier undertaking). See
  `integration_test/README.md`.

**No embedded Linux userspace, PRoot, or bundled shell/distro was
introduced.** The evidence so far doesn't show one is needed for system
binaries (they already execute directly and fine) — see section 3/11 for
why that question is specifically still open for *future downloaded
toolchain binaries*, not for the commands this phase verifies.
